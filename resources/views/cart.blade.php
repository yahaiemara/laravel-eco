        ...
        <div class="shopping-cart">
            ...
            <div class="cart-table__wrapper">
                        ...
                        <tr>
                            <td>
                                <div class="shopping-cart__product-item">
                                    <img loading="lazy" src="{{ asset('upload/product/thumbnails/' . $item->model->image) }}" width="120" height="120" alt="{{ $item->name }}" />
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="qty-control position-relative">
                                    ...
                                    <form action="{{ route('cart.qty.decrease', $item->rowId) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <input type="submit" class="qty-control__reduce" value="-">
                                    </form>
                                    <form action="{{ route('cart.qty.increase', $item->rowId) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <input type="submit" class="qty-control__increase" value="+">
                                    </form>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <form action="{{ route('cart.remove', $item->rowId) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="remove-cart" style="background: none; border: none; cursor: pointer; padding: 0;">
                ...
                <div class="cart-table-footer">
                    ...
                    <form action="{{ route('coupon.apply') }}" class="position-relative bg-body" method="POST">
                        @csrf
                    ...
                    @endif
                    <form action="{{ route('cart.destory') }}" method="POST">
                        @csrf
                        @method('DELETE')
